...
fn link_subcommand_exists() {
    Command::cargo_bin("pay")
        ...
        .stdout(predicate::str::contains("metamask"));
}

#[test]
fn link_rejects_missing_wallet_flag() {
    let mut cmd = Command::cargo_bin("pay").expect("binary not found");
    cmd.env("PAYSKILL_SIGNER_KEY", LOCAL_KEY);
    cmd.args(["link"]);
    cmd.assert()
...
#[test]
fn link_rejects_unknown_wallet() {
    let mut cmd = Command::cargo_bin("pay").expect("binary not found");
    cmd.env("PAYSKILL_SIGNER_KEY", LOCAL_KEY);
    cmd.args(["link", "--wallet", "trustwallet"]);
    cmd.assert()
        .failure()
        .stderr(predicate::str::contains("unknown wallet"));
}

#[test]
fn link_accepts_metamask_alias() {
    // Should fail with "not a terminal" (because assert_cmd pipes stderr),
    // but the wallet parsing itself should succeed.
    let mut cmd = Command::cargo_bin("pay").expect("binary not found");
    cmd.env("PAYSKILL_SIGNER_KEY", LOCAL_KEY);
    cmd.args(["link", "--wallet", "mm"]);
    let output = cmd.output().expect("failed to run");
    let stderr = String::from_utf8_lossy(&output.stderr);
    // Should NOT contain "unknown wallet" — alias should be recognized
    assert!(
        !stderr.contains("unknown wallet"),
        "mm alias should be recognized, got: {stderr}"
    );
}

#[test]
fn link_accepts_coinbase_alias() {
    let mut cmd = Command::cargo_bin("pay").expect("binary not found");
    cmd.env("PAYSKILL_SIGNER_KEY", LOCAL_KEY);
    cmd.args(["link", "--wallet", "cb"]);
    let output = cmd.output().expect("failed to run");
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        !stderr.contains("unknown wallet"),
        "cb alias should be recognized, got: {stderr}"
    );
}

#[test]
fn link_accepts_phantom() {
    let mut cmd = Command::cargo_bin("pay").expect("binary not found");
    cmd.env("PAYSKILL_SIGNER_KEY", LOCAL_KEY);
    cmd.args(["link", "--wallet", "phantom"]);
    let output = cmd.output().expect("failed to run");
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        !stderr.contains("unknown wallet"),
        "phantom should be recognized, got: {stderr}"
    );
}

#[test]
fn link_requires_interactive_terminal() {
    // assert_cmd runs with piped stderr (not a terminal), so link should fail
    // with an interactive terminal error, NOT a wallet parsing error
    ensure_local_init();
    let mut cmd = Command::cargo_bin("pay").expect("binary not found");
    cmd.env("PAYSKILL_SIGNER_KEY", LOCAL_KEY);
    cmd.args(["link", "--wallet", "metamask"]);
    cmd.assert()
        .failure()
...
#[test]
fn link_requires_init() {
    // Without any key configured, should fail with "not initialized" or "interactive terminal"
    let mut cmd = Command::cargo_bin("pay").expect("binary not found");
    cmd.env_remove("PAYSKILL_SIGNER_KEY");
    // Use a temp dir so no existing .meta/.enc files are found
    cmd.env("PAY_KEYS_DIR", std::env::temp_dir().join("pay-link-test-nonexistent"));
    cmd.args(["link", "--wallet", "metamask"]);
    cmd.assert().failure();
}

#[test]
fn link_json_mode_outputs_address_without_key() {
    // --json mode should output address info without requiring interactive terminal
    ensure_local_init();
    let mut cmd = Command::cargo_bin("pay").expect("binary not found");
    cmd.env("PAYSKILL_SIGNER_KEY", LOCAL_KEY);
    cmd.args(["--json", "link", "--wallet", "metamask"]);
    let output = cmd.output().expect("failed to run");
    ...
    if output.status.success() {
        ...
        assert!(parsed["address"].is_string(), "should contain address");
        assert_eq!(parsed["wallet"].as_str(), Some("MetaMask"));
        assert_eq!(parsed["network"].as_str(), Some("Base"));
        assert_eq!(parsed["chain_id"].as_u64(), Some(8453));
    }
    // If it fails (interactive terminal check before JSON), that's also acceptable
}
